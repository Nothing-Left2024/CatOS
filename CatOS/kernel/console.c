...
PUBLIC void init_screen(TTY* p_tty)
{
	...
	int con_v_mem_size			= v_mem_size / NR_CONSOLES;		/* 每个控制台占显存大小		(in WORD) */
	p_tty->p_console->original_addr		= nr_tty * con_v_mem_size;		/* 当前控制台占显存开始地址		(in WORD) */
	p_tty->p_console->v_mem_limit		= con_v_mem_size / SCREEN_WIDTH * SCREEN_WIDTH;			/* 当前控制台占显存大小		(in WORD) */
	p_tty->p_console->current_start_addr	= p_tty->p_console->original_addr;	/* 当前控制台显示到显存什么位置	(in WORD) */

	p_tty->p_console->cursor = p_tty->p_console->original_addr;	/* 默认光标位置在最开始处 */

	if (nr_tty == 0) {
		p_tty->p_console->cursor = disp_pos / 2;	/* 使用内核设置的光标位置 */
		/* disp_pos = 0; <-- 已注释! 避免覆盖kernel.asm设置的值 */
	}
	else {
...
/*======================================================================*
                           out_char_color
 *----------------------------------------------------------------------*
 * 与 out_char 相同, 但使用指定的属性 attr (而非 DEFAULT_CHAR_COLOR).
 * 供 set_color / putc_color 等系统调用使用.
 *======================================================================*/
...
PUBLIC void console_clrscr(CONSOLE* p_con)
{
	...
	int i;
	int n = p_con->v_mem_limit;	/* 单位: WORD */
...
/*======================================================================*
                           console_gotoxy
 *----------------------------------------------------------------------*
 * 将光标移动到相对控制台左上角的 (x, y) 字符坐标 (越界则截断).
 *======================================================================*/
PUBLIC void console_gotoxy(CONSOLE* p_con, int x, int y)
{
	...
	if (pos < 0) pos = 0;
	if (pos >= p_con->v_mem_limit) pos = p_con->v_mem_limit - 1;

	p_con->cursor = p_con->original_addr + pos;
